...
export default function VideoPlayer() {
  ...
  const { data: comments = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Comment.filter({ video_id: videoId }, '-created_date'),
    enabled: !!videoId,
  });

  const { data: relatedVideos = [] } = useQuery({
    queryKey: ['relatedVideos', video?.topic, video?.grade],
    queryFn: () => base44.entities.Video.filter({ topic: video?.topic, grade: video?.grade }, '-created_date', 5),
    enabled: !!video?.topic,
  });
  ...
  useEffect(() => {
    ...
    base44.functions.invoke('trackVideoView', { video_id: video.id })
      .catch(err => console.error('Failed to track view:', err));

    // Award XP for watching — deduplicate by checking existing events
    base44.entities.XPEvent.filter({ user_email: user.email, reference_id: video.id, action_type: 'video_watch' })
      .then(existing => {
